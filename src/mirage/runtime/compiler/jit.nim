## Utilities for the JIT compiler
...
proc verifySize*(size: uint) {.inline.} =
...
proc jitAlloc*(size: uint): pointer =
  verifySize(size)
...
proc jitFree*(memory: pointer, size: uint) =
  verifySize(size)
...
proc markExecutable*(memory: pointer, size: uint) =
  if size == 0:
    return
